...
cc.Class({
    ...
    createList(data, jsName) {
        this.data = data;
        this.jsName = jsName
        this.itemHeight = cc.instantiate(this.item).height;//设置每个item的高
        this.topIndex = 0;//最上面的item索引id
        let index = Math.floor(this.scrollView.height / this.itemHeight) + 2
        let freshCOunt = index + 1
        if (index > data.length) {
            index = data.length
            freshCOunt = index
        }
        this.bottomIndex = index;//最下面的item索引id
        this.offsetY = this.itemHeight / 2;//上下临界坐标补充，点击查看原理，值太小会出现item在边界闪动的情况，因为item可能此时在上下两边时均符合移动的情况，所以就会无限循环移动，此值建议自行调整
        ...
        this.content.height = (data.length) * this.itemHeight;
    },

    updateItem(listItem, data, i) {
        listItem.y = -i * this.itemHeight - this.itemHeight / 2;
        listItem.getComponent(this.jsName).updateUI(data);
